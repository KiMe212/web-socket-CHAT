...
async def create_room(
    ...
    session: SessionLocal = Depends(db_session_dependency),
):
    if user:
        # check that room isn't exist
        data_query = select(Room.id).where(Room.name == new_room.name)
...
async def delete_room(
    ...
    if user:
        ...
        result = await session.execute(select(Room.user_id).where(Room.name == room.name))
...
async def get_all_rooms(session: SessionLocal = Depends(db_session_dependency)):
...
async def websocket_endpoint(
    ...
    if user:
        # check the room
        stmt = select(Room.name, Room.id).where(Room.name == room)
        result = await session.execute(stmt)
        data_room = result.mappings().first()
        if room == data_room["name"]:
            ...
            # get old messages
            stmt = select(Message.user_name, Message.message).where(
                Message.room_id == data_room["id"]
            )
            result = await session.execute(stmt)
            old_message = result.mappings().all()
